---
title: "Validating Hardware We Don't Have Yet"
date: 2025-10-04
author: Kevin Griffin
tags: [ar-glasses, hardware, thermal, ci, ar2-gen1]
description: "This week the runtime pivoted from AR1+ to AR2 Gen1, and started shipping thermal-validation, Wi-Fi 7 stability tests, and a sensor-integration bring-up harness for hardware that does not exist on my desk. Here is why that is a sensible thing to do."
series: learning-to-code-with-ai
slug: ar2-gen1-validating-hardware-we-dont-have
---

There is a stage every hardware product goes through where the spec exists, the validation plan exists, the test harness exists, and the actual silicon does not. The traditional answer is to sit and wait. The modern answer is to build everything that is downstream of the hardware first, so that when the hardware shows up, the validation begins on day one.

This week was that. The target platform officially shifted from AR1+ to AR2 Gen1, and the engineering response was to immediately build the validation infrastructure the new platform will need, before any of us touch the new platform.

## What landed

Five things, all landed this week:
...
This was a 77-commit week. Most of those commits are in service of a device I cannot hold.

## Why this is sensible

A few reasons.

**Hardware validation that ships with the hardware is hardware validation that lands six months late.** The thermal-and-power framework written this week will be used the day the first AR2 dev kit reaches my desk, not three months after. The platform's first thermal regression will be caught by an existing test, not by a human noticing the device is hot.
...
This is the one I am proudest of from this week.

The AR2 Gen1 spec assumes Wi-Fi 7 connectivity for offloaded rendering and tethered compute. Wi-Fi 7 in practice is not as stable as Wi-Fi 6E, and the failure modes are different. The test suite written this week characterizes the suite of failure modes (degraded throughput, intermittent loss, reauth storms) and bounds the runtime's behavior under each. The runtime cannot tolerate every failure. The job of the test suite is to be specific about which failures the runtime gracefully degrades through and which fail loud.

This matters because the AR experience the user is having cannot stutter when the connection blips. The runtime has to fall back to local rendering for a frame, or two, or twenty, depending on the duration of the blip, and re-converge when connectivity returns. That logic existed in fuzzy intent in the spec. After this week, it exists as test cases.
...
77 commits, week five. The week ended with documentation, which is the right kind of week to end with.
