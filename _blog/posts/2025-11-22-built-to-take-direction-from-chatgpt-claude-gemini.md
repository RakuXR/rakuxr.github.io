...
tags: [llm, partnerships, voice-pipeline, openxr, xr-assistant, ai-native]
description: "This week the runtime got an XRAssistantService that integrates cloud LLM responses into the voice pipeline, plus OpenXR foundations, eye-tracked foveated rendering, full-body tracking, and a Wi-Fi 7 offloaded rendering path. The engine is being built to be driven by the best model available, whoever's model that is."
...
That is not what Raku is doing. Raku is being built to take direction from a cloud LLM as a runtime concern, not an authoring convenience. This week's work made that explicit.

## What landed

A hundred and eighteen commits across the runtime this week. The headline pieces:
...
The interface is model-agnostic by design. ChatGPT can drive this. Claude can drive this. Gemini can drive this. A small on-device model can drive this. Anything that produces a stream of intent tokens that conforms to the assistant-service protocol can drive this. The piece we built this week is the integration layer between any such model and the rest of the runtime.
...
This week the offloaded rendering pipeline landed end-to-end. The host server runs anywhere. The runtime on the glasses talks to the host over Wi-Fi 7. Frame latency is monitored and the runtime can gracefully degrade to local rendering when the link blips, which it will.
...
Hundred and eighteen commits, big week. The engine looks different at the end of it than it did at the start.
