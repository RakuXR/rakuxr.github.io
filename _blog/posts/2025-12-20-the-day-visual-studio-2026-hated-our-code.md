---
title: "The Day Visual Studio 2026 Hated Our Code"
date: 2025-12-20
author: Kevin Griffin
tags: [build, msvc, vs2026, openxr, debugging, dev-workflow]
description: "The runtime built clean on Linux. The runtime built clean on macOS. Then I tried to build it in Visual Studio 2026 Insiders, and Visual Studio 2026 had opinions. Twelve PRs later, the build is green. Here is the inside of those twelve PRs."
series: learning-to-code-with-ai
slug: the-day-visual-studio-2026-hated-our-code
---

There is an old joke that the difference between a senior engineer and a junior engineer is that the senior engineer knows it is going to be a fight with the compiler before they start. This week was a fight with the compiler. The compiler was Microsoft Visual C++ as it ships in Visual Studio 2026 Insiders. The codebase was Raku. The fight lasted four days. The codebase won, but barely.
...
Up to this week the runtime built clean on Linux with GCC and on macOS with Clang. Both of those have been my main development environments. I had not actually tried a Windows MSVC build since the early bring-up in October. The runtime is supposed to be cross-platform. Cross-platform means Windows. So I checked out the repo on a Windows machine with Visual Studio 2026 Insiders and ran the build.
...
Twelve PRs in four days, give or take. Every one of them is documented under #404–#416 in the runtime repo.

The second day was the longest. The DLL-export normalization required reading every public-API header in the engine and deciding which symbols were genuinely part of the public surface. Some of them turned out not to be. A few got demoted to internal as part of this work, which was a net good even though it added scope.

The third day I added a Windows MSVC CI workflow with vcpkg and OpenXR SDK integration so that this never happens again silently. Now every push triggers an MSVC build. The build cannot regress without somebody seeing it in CI.

The fourth day was the celebration. By the end of it, the build went green on Windows. First successful x64 build in the runtime's history. The commit message says exactly that: `build: first successful x64 build with OpenXR handle fixes`. The commit hash is `f35f78bd` and it is one of my favorite commits in the project to date.

## What worked, what I would do differently

A few honest notes.

**Bringing a new platform online late is expensive.** I should have run an MSVC build in October when the codebase was smaller. The errors at that scale would have been twenty, not two hundred. The cost of fixing twenty errors is meaningfully smaller than the cost of fixing two hundred. The lesson is to never let a target platform go un-built for more than a few weeks.
...
**A different model caught a bug another model wrote.** During the deprecated-CRT replacement sweep, one of the agents replaced `sprintf` with `sprintf_s` in a place where the buffer-size semantics were subtly different from what the call site expected. The PR landed. A review pass by a different model caught the mismatch before it shipped. The multi-vendor review workflow earned its keep this week, specifically.
...
Saturday morning, the engine builds clean on three platforms. That is the right thing to start the next week with.
